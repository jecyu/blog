...
使用 [Node.js](https://nodejs.org/en/) 不仅仅是实现后端应用，还实现了整个 HTTP 服务器，服务器可以监听客户端的请求，类似 Apache、Nginx 等 HTTP 服务器。首先把 Node.js 安装到 /usr/local 目录下，然后分别为 `node` 和 `npm` 命令建立软连接，以便在任何地方都可以调用它们。
